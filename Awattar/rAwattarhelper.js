...
if (price_today_min > (aWATTar_Stundenpreise_Today05_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today05_Totalgross.state as Number).doubleValue hour_today_min = 5 }
if (price_today_min > (aWATTar_Stundenpreise_Today06_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today06_Totalgross.state as Number).doubleValue hour_today_min = 6 }
if (price_today_min > (aWATTar_Stundenpreise_Today07_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today07_Totalgross.state as Number).doubleValue hour_today_min = 7 }
if (price_today_min > (aWATTar_Stundenpreise_Today08_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today08_Totalgross.state as Number).doubleValue hour_today_min = 8 }
if (price_today_min > (aWATTar_Stundenpreise_Today09_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today09_Totalgross.state as Number).doubleValue hour_today_min = 9 }
if (price_today_min > (aWATTar_Stundenpreise_Today10_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today10_Totalgross.state as Number).doubleValue hour_today_min = 10 }
if (price_today_min > (aWATTar_Stundenpreise_Today11_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today11_Totalgross.state as Number).doubleValue hour_today_min = 11 }
if (price_today_min > (aWATTar_Stundenpreise_Today12_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today12_Totalgross.state as Number).doubleValue hour_today_min = 12 }
if (price_today_min > (aWATTar_Stundenpreise_Today13_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today13_Totalgross.state as Number).doubleValue hour_today_min = 13 }
if (price_today_min > (aWATTar_Stundenpreise_Today14_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today14_Totalgross.state as Number).doubleValue hour_today_min = 14 }
if (price_today_min > (aWATTar_Stundenpreise_Today15_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today15_Totalgross.state as Number).doubleValue hour_today_min = 15 }
if (price_today_min > (aWATTar_Stundenpreise_Today16_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today16_Totalgross.state as Number).doubleValue hour_today_min = 16 }
if (price_today_min > (aWATTar_Stundenpreise_Today17_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today17_Totalgross.state as Number).doubleValue hour_today_min = 17 }
if (price_today_min > (aWATTar_Stundenpreise_Today18_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today18_Totalgross.state as Number).doubleValue hour_today_min = 18 }
if (price_today_min > (aWATTar_Stundenpreise_Today19_Totalgross.state as Number)) { price_today_min = (aWATTar_Stundenpreise_Today19_Totalgross.state as Number).doubleValue hour_today_min = 19 }
...
price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow00_Totalgross.state as Number).doubleValue
  ...
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow05_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow05_Totalgross.state as Number).doubleValue hour_tomorrow_min = 5 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow06_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow06_Totalgross.state as Number).doubleValue hour_tomorrow_min = 6 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow07_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow07_Totalgross.state as Number).doubleValue hour_tomorrow_min = 7 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow08_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow08_Totalgross.state as Number).doubleValue hour_tomorrow_min = 8 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow09_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow09_Totalgross.state as Number).doubleValue hour_tomorrow_min = 9 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow10_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow10_Totalgross.state as Number).doubleValue hour_tomorrow_min = 10 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow11_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow11_Totalgross.state as Number).doubleValue hour_tomorrow_min = 11 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow12_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow12_Totalgross.state as Number).doubleValue hour_tomorrow_min = 12 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow13_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow13_Totalgross.state as Number).doubleValue hour_tomorrow_min = 13 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow14_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow14_Totalgross.state as Number).doubleValue hour_tomorrow_min = 14 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow15_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow15_Totalgross.state as Number).doubleValue hour_tomorrow_min = 15 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow16_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow16_Totalgross.state as Number).doubleValue hour_tomorrow_min = 16 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow17_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow17_Totalgross.state as Number).doubleValue hour_tomorrow_min = 17 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow18_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow18_Totalgross.state as Number).doubleValue hour_tomorrow_min = 18 }
  if (price_tomorrow_min > (aWATTar_Stundenpreise_Tomorrow19_Totalgross.state as Number)) { price_tomorrow_min = (aWATTar_Stundenpreise_Tomorrow19_Totalgross.state as Number).doubleValue hour_tomorrow_min = 19 }
